  ...
  return (
    <div style={{ padding: "36px 40px", background: "#000", minHeight: "100vh" }}>
      <div style={{ marginBottom: 36 }}>
        ...
        <p style={{ fontSize: 14, color: "#555", marginTop: 4 }}>{currentDate}</p>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16, marginBottom: 40 }}>
        {stats.map((s, i) => (
          <div key={i} className="card-hover" style={{ background: "#0a0a0a", border: `1px solid ${s.border}`, borderRadius: 14, padding: "24px 22px", cursor: "default" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        <div style={{ background: "#0a0a0a", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, overflow: "hidden" }}>
          <div style={{ padding: "16px 20px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <Link href="/campaigns/active" style={{ fontSize: 12, color: "#22c00d", textDecoration: "none", fontWeight: 500 }}>View all →</Link>
          </div>
          {active.length === 0 ? (
            ...
            active.slice(0, 5).map(c => (
              <div key={c.id} style={{ padding: "13px 20px", borderBottom: "1px solid rgba(255,255,255,0.04)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <span className="badge" style={{ color: priorityColor[c.priority] || "#555", background: `${priorityColor[c.priority] || "#555"}15`, border: `1px solid ${priorityColor[c.priority] || "#555"}30` }}>{c.priority}</span>
              </div>
            ))
          )}
        </div>
        ...
        <div style={{ background: "#0a0a0a", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 14, overflow: "hidden" }}>
          <div style={{ padding: "16px 20px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <Link href="/campaigns/completed" style={{ fontSize: 12, color: "#22c00d", textDecoration: "none", fontWeight: 500 }}>View all →</Link>
          </div>
          {completed.length === 0 ? (
            <div className="empty-state">
              <div className="empty-state-icon">🏁</div>
            ...
            completed.slice(0, 5).map(c => (
              <div key={c.id} style={{ padding: "13px 20px", borderBottom: "1px solid rgba(255,255,255,0.04)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "#fff", marginBottom: 3 }}>{c.title}</div>
                  <div style={{ fontSize: 11, color: "#555" }}>Completed {c.completed_at ? new Date(c.completed_at).toLocaleDateString("en-US", { month: "short", day: "numeric" }) : "—"}</div>
                </div>
