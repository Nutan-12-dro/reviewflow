            ...
            <div style={{ display:"flex", flexDirection:"column", gap:8, marginBottom:16 }}>
              <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", fontSize:13 }}>
                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                  ...
                  ) : <span style={{ fontWeight:500, color:"#a3a3a3" }}>{c.reviewer}</span>}
                </div>
                {editingReviewer!==c.id && (
                  <button onClick={()=>setEditingReviewer(c.id)} className="btn-ghost" style={{ fontSize:11, fontWeight:600, borderRadius:6, padding:"3px 9px", cursor:"pointer", color: "#22c00d" }}>↺ Change</button>
                )}
              </div>
              <div style={{ display:"flex", alignItems:"center", gap:8, fontSize:13, color:"#a3a3a3" }}>
                <span>💰</span><span style={{ color:"#a3a3a3" }}>{c.budget}</span>
              </div>
              <div style={{ display:"flex", alignItems:"center", gap:8, fontSize:13, color:"#a3a3a3" }}>
      ...
      {deleteId && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.8)", backdropFilter:"blur(4px)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center" }}>
          <div className="modal-enter" style={{ background:"#0a0a0a", border:"1px solid rgba(239,68,68,0.2)", borderRadius:16, padding:28, width:400 }}>
            ...
            <div style={{ display:"flex", gap:10, justifyContent:"flex-end", marginTop:24 }}>
              <button onClick={()=>setDeleteId(null)} style={{ padding:"9px 18px", background:"none", border:"1px solid rgba(255,255,255,0.1)", borderRadius:10, color:"#a3a3a3", cursor:"pointer" }}>Cancel</button>
              <button onClick={async ()=>{await deleteCampaign(deleteId);setDeleteId(null);}} style={{ padding:"9px 20px", background:"#ef4444", border:"none", borderRadius:10, color:"#fff", fontWeight:700, cursor:"pointer" }}>Delete</button>
            </div>
          </div>
        </div>
      )}
